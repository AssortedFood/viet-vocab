...
export async function editVocab(id, word, translation, description = "", category = "") {
  ...
  try {
    ...
    console.error("OpenAI correction failed:", error);
    correctedVocab = { word, translation, description, category }; // Use original values if API fails
  }

  // Step 3: Update the database entry
  const result = await db.run(
    `UPDATE vocab SET word = ?, translation = ?, description = ?, category = ? WHERE id = ?`,
    [correctedVocab.word, correctedVocab.translation, correctedVocab.description, correctedVocab.category, id]
  );
